...
class VerticalLandingEnv(gym.Env):
    ...
    def render(self, mode='human', close=False):
        ...
        if self.viewer is None:
            ...
            self.fire = rendering.FilledPolygon(((ENGINE_WIDTH * 0.4, 0),
                                                 (-ENGINE_WIDTH * 0.4, 0),
        ...
        if self.stepnumber % round(FPS / 10) == 0 and self.power > 0:
            # total_lifetime = MAX_SMOKE_LIFETIME * self.power
            # current_lifetime = 0
            # size = self.power * (1 + 0.2 * np.random.random())
            # s = [total lifetime, current lifetime, size, np.array(self.lander.position) + self.power * ROCKET_WIDTH * 10 * np.array((np.sin(self.lander.angle + self.gimbal), -np.cos(self.lander.angle + self.gimbal))) + self.power * 5 * (np.random.random(2) - 0.5)]    # position
        ...
        for l in zip(self.legs, [-1, 1]):
            path = [self.lander.fixtures[0].body.transform * (l[1] * ROCKET_WIDTH / 2,
                    ROCKET_HEIGHT / 8),
    ...
    def reset(self):
        ...
        self.lander.linearVelocity = (-self.np_random.uniform(0, INITIAL_RANDOM) * START_SPEED * (initial_x - W / 2) / W, -START_SPEED)
